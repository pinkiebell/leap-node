...
describe('getColor', () => {
  test('erc20 color', async () => {
    expect(await getColor({ contract, tokens }, erc20Tokens[0])).toBe('0x0');
    expect(await getColor({ contract, tokens }, erc20Tokens[1])).toBe('0x1');
  });

  test('erc721 color', async () => {
    expect(await getColor({ contract, tokens }, erc721Tokens[0])).toBe(
      `0x${NFT_COLOR_BASE.toString(16)}`
    );
    expect(await getColor({ contract, tokens }, erc721Tokens[1])).toBe(
